...
void WriteKittiPoseText(std::ofstream &ofs, double time, const Eigen::Matrix3d &rot, const Eigen::Vector3d &pos) {
    ...
    ofs << std::fixed << std::setprecision(6) << time << " " << rpy[0] << " " << rpy[1] << " " << rpy[2] << " "
...
class MapBuilderROS {
   public:
    ...
    void initPublishers() {
        local_cloud_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("local_cloud", 100);
        body_cloud_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("body_cloud", 100);
        ...
        local_path_pub_ = nh_.advertise<nav_msgs::Path>("local_path", 100);
        global_path_pub_ = nh_.advertise<nav_msgs::Path>("global_path", 100);
    }

    void initServices() { save_map_server_ = nh_.advertiseService("save_map", &MapBuilderROS::saveMapCallback, this); }
    ...
    void publishLocalPath() {
        ...
        if (shared_data_->key_poses.empty()) return;
    ...
    void publishLoopMark() {
        ...
        if (shared_data_->loop_history.empty()) return;
    ...
    bool saveMapCallback(fastlio::SaveMap::Request &req, fastlio::SaveMap::Response &res) {
        ...
        std::string pts_save_path = save_dir + "/map_" + time_str + ".pcd";
        LOG_INFO("Save map to %s.", pts_save_path.c_str());

        // *保存点云
        bool down_size = resolution > 1e-6;
        ...
        for (const Pose6D &p : shared_data_->key_poses) {
            ...
            pcl::transformPointCloud(*shared_data_->cloud_history[p.index], *temp_cloud, p.global_pos.cast<float>(),
        ...
        if (!air_slam::FileSystem::createDirectory(save_dir)) {
            res.status = false;
            res.message = "Create directory failed";
            LOG_ERROR("Create directory %s failed!", save_dir.c_str());
            return false;
        }
        writer_.writeBinaryCompressed(pts_save_path, *cloud);
        LOG_INFO("Save %ld pts to %s success.", cloud->points.size(), pts_save_path.c_str());

        // *保存轨迹
        std::string odom_traj_save_path = save_dir + "/odom-traj_" + time_str + ".txt";
        std::string slam_traj_save_path = save_dir + "/slam-traj_" + time_str + ".txt";

        std::ofstream odom_stream;
        std::ofstream slam_stream;
        air_slam::FileSystem::CreateFile(odom_stream, odom_traj_save_path);
        air_slam::FileSystem::CreateFile(slam_stream, slam_traj_save_path);

        for (const Pose6D &p : shared_data_->key_poses) {
            WriteKittiPoseText(odom_stream, p.time, p.local_rot, p.local_pos);
            WriteKittiPoseText(slam_stream, p.time, p.global_rot, p.global_pos);
        }
        odom_stream.close();
        slam_stream.close();
        res.status = true;
        res.message = "Save map success!";
        LOG_INFO("Save %ld poses.", shared_data_->key_poses.size());
        return true;
    }
    ...
    void run() {
        ...
        while (ros::ok()) {
            ...
            if (!measure_group_.syncPackage(imu_data_, rs_m1_data_)) continue;
            lio_builder_->mapping(measure_group_);
            if (lio_builder_->currentStatus() == fastlio::Status::INITIALIZE) continue;
            current_time_ = measure_group_.lidar_time_end;
            current_state_ = lio_builder_->currentState();

            br_.sendTransform(eigen2Transform(shared_data_->offset_rot, shared_data_->offset_pos, global_frame_,
                                              local_frame_, current_time_));
            br_.sendTransform(eigen2Transform(current_state_.rot.toRotationMatrix(), current_state_.pos, local_frame_,
                                              body_frame_, current_time_));

            publishOdom(eigen2Odometry(current_state_.rot.toRotationMatrix(), current_state_.pos, local_frame_,
                                       body_frame_, current_time_));

            // todo::长距离运行爆内存风险
            addKeyPose();

            publishCloud(body_cloud_pub_, pcl2msg(lio_builder_->cloudUndistortedBody(), body_frame_, current_time_));
   ...
   private:
    void addKeyPose() {
        ...
        if (shared_data_->key_poses.empty()) {
            ...
            shared_data_->key_poses.emplace_back(idx, current_time_, current_state_.rot.toRotationMatrix(),
                                                 current_state_.pos);
        ...
        Eigen::Vector3d diff_pose =
            last_key_pose.local_rot.transpose() * (current_state_.pos - last_key_pose.local_pos);
