...
namespace fastlio
{
    ...
    void LoopClosureThread::setShared(const std::shared_ptr<SharedData> &share_data)
    {
        shared_data_ = share_data;
    }

    void LoopClosureThread::setRate(double rate)
    {
        update_rate_ = 1.0 / (rate + 1e-5);
    }

    LoopParams &LoopClosureThread::mutableParams()
    {
        return loop_params_;
    }
