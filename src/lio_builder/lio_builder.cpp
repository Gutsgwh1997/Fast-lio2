...
void LIOBuilder::trimMap() {
    ...
    float mov_dist = std::max((local_map_.cube_len - 2.0 * det_thresh) * 0.5 * 0.9,
...
void LIOBuilder::increaseMap() {
    if (status_ == Status::INITIALIZE) return;
    if (cloud_down_lidar_->empty()) return;

    // todo::确保点云的数量不会超过最大值NUM_MAX_POINTS
    ...
    for (int i = 0; i < size; i++) {
        ...
        mid_point.x = std::floor(cloud_down_world_->points[i].x / params_.resolution) * params_.resolution +
                      0.5 * params_.resolution;
        mid_point.y = std::floor(cloud_down_world_->points[i].y / params_.resolution) * params_.resolution +
                      0.5 * params_.resolution;
        mid_point.z = std::floor(cloud_down_world_->points[i].z / params_.resolution) * params_.resolution +
                      0.5 * params_.resolution;

        // 如果该点所在的voxel没有点，则直接加入地图，且不需要降采样
        if (fabs(points_near[0].x - mid_point.x) > 0.5 * params_.resolution &&
            fabs(points_near[0].y - mid_point.y) > 0.5 * params_.resolution &&
            fabs(points_near[0].z - mid_point.z) > 0.5 * params_.resolution) {
...
#pragma omp parallel for
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (!point_selected_flag_[i]) continue;
