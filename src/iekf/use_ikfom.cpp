...
namespace air_slam
{
    ...
    namespace esekfom
    {
        ...
        Eigen::Matrix<double, 24, 24> df_dx(const state_ikfom &s, const input_ikfom &in)
        {
            ...
            Eigen::Vector3d acc = in.acc - s.ba;                  // 测量加速度 = a_m - bias

            cov.block<3, 3>(12, 3) = -s.rot.matrix() * Sophus::SO3::hat(acc); // 对应公式(7)第3行第1列
            cov.block<3, 3>(12, 18) = -s.rot.matrix();                        // 对应公式(7)第3行第5列

            cov.block<3, 3>(12, 21) = Eigen::Matrix3d::Identity(); // 对应公式(7)第3行第6列   I
            cov.block<3, 3>(3, 15) = -Eigen::Matrix3d::Identity(); // 对应公式(7)第1行第4列 (简化为-I)
            return cov;
        }
